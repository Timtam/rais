...
//!
//! `apply_self_update` invokes `verify_executable_signature` on each binary
//! that will replace an install target. Three outcomes:
//!
...
//! - `Invalid` — the binary claims to be signed but verification failed
//!   (tampering, expired chain, mismatched hash). Apply rejects this before
...
#[cfg(target_os = "macos")]
fn verify_executable_signature_impl(path: &Path) -> Result<SignatureVerdict> {
    ...
    let output = Command::new("/usr/bin/codesign")
        ...
        .arg(path)
        .output()
        .map_err(|source| crate::error::RaisError::Io {
...
fn verify_executable_signature_impl(_path: &Path) -> Result<SignatureVerdict> {
...
mod tests {
    ...
    fn classifies_signtool_success_as_signed() {
        let verdict =
            verdict_from_signtool_output(exit_status(0), "Successfully verified: RAIS.exe", "");
        assert!(matches!(verdict, SignatureVerdict::Signed { .. }));
    }
