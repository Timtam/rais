...
/// Per-target lock path: `<resource_path>/RABBIT/locks/package-install.lock`.
///
/// RABBIT used to drop this under `%LOCALAPPDATA%\RABBIT\locks\` (and
/// `~/Library/Caches/RABBIT/locks/` on macOS) which made the otherwise
/// portable RABBIT workflow leave persistent files behind in the host's
...
/// that already lives there, so RABBIT writes nothing outside the install
/// target the user picked.
///
/// Two RABBIT processes installing into *different* resource paths can now
...
pub fn acquire_package_install_lock_at(path: &Path) -> Result<PackageInstallLock> {
    ...
    if path.exists() {
        match read_lock_holder(path) {
            Some(holder) if pid_is_alive(holder.pid) => {
                return Err(RabbitError::PackageInstallInProgress {
    ...
    let serialized = serde_json::to_string(&metadata).map_err(|source| RabbitError::Json {
...
mod tests {
    ...
    fn second_acquire_fails_when_holder_is_alive() {
        ...
        assert!(matches!(
            error,
            RabbitError::PackageInstallInProgress { .. }
        ));
    }
